...
const Header = ({ loader }: HeaderProps) => {
  ...
  return (
    <motion.header
      className={cn(
        styles.header,
        "transition-colors delay-100 duration-500 ease-in pointer-events-none"
      )}
      style={{
    ...
    >
      ...
      <div className={cn(styles.bar, "flex items-center justify-between")}>
        ...
        <OnlineUsers />
        <FunnyThemeToggle className="w-6 h-6 mr-4 pointer-events-auto" />
        <Button
          variant={"ghost"}
          onClick={() => setIsActive(!isActive)}
          className={cn(
            styles.el,
            "m-0 p-0 h-6 bg-transparent flex items-center justify-center pointer-events-auto"
          )}
        >
